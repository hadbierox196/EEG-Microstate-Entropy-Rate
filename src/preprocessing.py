...
def preprocess_subject(raw_path, file_format='auto'):
    ...
    # Zero-phase FIR bandpass, matching the manuscript (Section 2.2).
    # Do NOT use method='iir' here — that has a non-zero phase response
    # and is not what was reported.
    raw.filter(l_freq=1.0, h_freq=40.0, method='fir', phase='zero', verbose=False)
